...
const (
	...
	IMPLIES  LinkType = "IMPLIES"
)

type CardType string
const (
	Thesis   CardType = "Thesis"
	Evidence CardType = "Evidence"
)
...
// just reads a yaml file and parses it.
func LoadDeckFromFile(filePath string) (*Deck, error) {
	...
	if err := yaml.Unmarshal(data, &d); err != nil {
		return nil, fmt.Errorf("failed to unmarshal deck YAML: %w", err)
	}
	d.buildMaps()
	return &d, nil
}
...
func (d *Deck) buildMaps() {
	d.CardMap = make(map[string]*Card)
	for _, card := range d.Cards {
		d.CardMap[card.ID] = card
	}
	d.ConceptMap = make(map[string]*Concept)
	for i := range d.Concepts {
		d.ConceptMap[d.Concepts[i].ID] = &d.Concepts[i]
	}
}